...
var canvas_inline_default = 'function Be(){let Y=document.querySelectorAll(".canvas-container");if(Y.length!==0)for(let t of Array.from(Y)){if(t.dataset.initialized==="true")continue;t.dataset.initialized="true";let z=t.querySelector(".canvas-viewport");if(!z)continue;let He=t.dataset.enableInteraction!=="false",Xe=t.dataset.defaultFullscreen==="true",p=t.closest(\'.page[data-frame="canvas"]\'),Se=window.matchMedia("(max-width: 800px)").matches;p&&!Xe&&!Se&&(p.classList.add("canvas-init-no-transition","canvas-sidebar-open"),p.offsetWidth,requestAnimationFrame(()=>{p.classList.remove("canvas-init-no-transition")}));let H=parseFloat(t.dataset.minZoom??"")||.1,X=parseFloat(t.dataset.maxZoom??"")||5,Ie=parseFloat(t.dataset.initialZoom??"")||1,s=1,f=s,R=0,W=0,d=0,y=0,a=0,i=0,k=!1,se=0,re=0,g=()=>{z.style.transform=`translate(${a}px, ${i}px) scale(${s})`},ae=.9,ie=()=>{let n=t.getBoundingClientRect(),c=parseFloat(z.style.width)||1e3,l=parseFloat(z.style.height)||1e3,w=n.width/c*ae,F=n.height/l*ae,v=Math.min(w,F,1)*Ie;return v=Math.max(H,Math.min(X,v)),{zoom:v,panX:(n.width-c*v)/2,panY:(n.height-l*v)/2}},b=()=>{y&&(cancelAnimationFrame(y),y=0)},K=()=>{let n=ie();s=n.zoom,a=n.panX,i=n.panY,f=s,d&&(cancelAnimationFrame(d),d=0),b(),g()};K();let S=s,I=a,A=i,C=t.querySelector(".canvas-reset-view"),ce=()=>Math.abs(s-S)<=.001&&Math.abs(a-I)<=1&&Math.abs(i-A)<=1,L=()=>{C&&(C.style.display=ce()?"none":"flex")},_=.2,$=()=>{let n=s;s+=(f-s)*_,Math.abs(f-s)<5e-4&&(s=f),a=R-(R-a)*(s/n),i=W-(W-i)*(s/n),g(),L(),d=s===f?0:requestAnimationFrame($)},U=(n,c,l)=>{b(),f=Math.max(H,Math.min(X,f*n)),R=c,W=l,d||(d=requestAnimationFrame($))},Z=1,B=0,D=0,le=()=>{s+=(Z-s)*_,a+=(B-a)*_,i+=(D-i)*_,Math.abs(Z-s)<5e-4&&Math.abs(B-a)<.5&&Math.abs(D-i)<.5&&(s=Z,a=B,i=D),f=s,g(),L(),y=s===Z&&a===B&&i===D?0:requestAnimationFrame(le)},j=(n,c,l)=>{Z=Math.max(H,Math.min(X,n)),B=c,D=l,d&&(cancelAnimationFrame(d),d=0),y||(y=requestAnimationFrame(le))},h=[],M=t.getBoundingClientRect(),Ae=()=>{let n=t.getBoundingClientRect();if(n.left===M.left&&n.top===M.top&&n.width===M.width&&n.height===M.height)return;let c=ce();a-=n.left-M.left,i-=n.top-M.top,M=n;let l=ie();S=l.zoom,I=l.panX,A=l.panY,c?K():g(),L()},ue=new ResizeObserver(Ae);if(ue.observe(t),h.push(()=>ue.disconnect()),He){let l=e=>e.deltaMode!==0?e.deltaY>0?.7692307692307692:1.3:Math.exp(-e.deltaY*.004),w=e=>e.deltaMode!==0?!1:e.deltaX!==0||!Number.isInteger(e.deltaY)?!0:Math.abs(e.deltaY)<40,F=(e,o)=>Math.abs(o.deltaX)>Math.abs(o.deltaY)?e.scrollWidth>e.clientWidth:e.scrollHeight>e.clientHeight,v=(e,o)=>Math.abs(o.deltaX)>Math.abs(o.deltaY)?!(e.scrollWidth<=e.clientWidth||e.scrollLeft<=0&&o.deltaX<0||e.scrollLeft+e.clientWidth>=e.scrollWidth-1&&o.deltaX>0):!(e.scrollHeight<=e.clientHeight||e.scrollTop<=0&&o.deltaY<0||e.scrollTop+e.clientHeight>=e.scrollHeight-1&&o.deltaY>0),P=!1,fe=e=>{if(P){e.preventDefault();return}let o=t.getBoundingClientRect(),r=e.clientX-o.left,u=e.clientY-o.top;if(e.ctrlKey||e.metaKey){e.preventDefault(),U(l(e),r,u);return}let m=e.target instanceof HTMLElement?e.target.closest(".canvas-node:not(.canvas-node-group)"):null;if(m){let E=m.querySelector(".canvas-node-content");if(E instanceof HTMLElement&&F(E,e)){if(v(E,e))return;e.preventDefault();return}}e.preventDefault(),w(e)?(b(),a-=e.deltaX,i-=e.deltaY,g(),L()):U(l(e),r,u)},me=e=>{let o=e.button===1;if(!(e.button!==0&&!o)){if(o)e.preventDefault();else if(e.target instanceof HTMLElement){if(e.target.closest("a")||e.target.closest("button"))return;let r=e.target.closest(".canvas-node-content");if(r&&r.scrollHeight>r.clientHeight){let u=r.getBoundingClientRect();if(e.clientX>=u.right-16)return}}b(),k=!0,se=e.clientX-a,re=e.clientY-i,t.style.cursor="grabbing",t.setPointerCapture(e.pointerId)}},ve=e=>{k&&(a=e.clientX-se,i=e.clientY-re,g(),L())},G=()=>{k=!1,t.style.cursor=""},he=e=>e.preventDefault();t.addEventListener("wheel",fe,{passive:!1}),t.addEventListener("pointerdown",me),t.addEventListener("pointermove",ve),t.addEventListener("pointerup",G),t.addEventListener("pointercancel",G),t.addEventListener("dragstart",he);let te=0,ne=0,oe=0,N=!1,Ee=e=>{if(e.length<2||!e[0]||!e[1])return 0;let o=e[0].clientX-e[1].clientX,r=e[0].clientY-e[1].clientY;return Math.sqrt(o*o+r*r)},pe=e=>{if(e.touches.length===2){let o=e.touches[0],r=e.touches[1];if(!o||!r)return;e.preventDefault(),b(),N=!0,k=!1,te=Ee(e.touches),ne=(o.clientX+r.clientX)/2,oe=(o.clientY+r.clientY)/2}},ge=e=>{if(e.touches.length===2&&N){let o=e.touches[0],r=e.touches[1];if(!o||!r)return;e.preventDefault();let u=Ee(e.touches),m=(o.clientX+r.clientX)/2,E=(o.clientY+r.clientY)/2,V=t.getBoundingClientRect(),q=m-V.left,x=E-V.top,T=u/te,Ye=s;s=Math.max(H,Math.min(X,s*T)),f=s,a=q-(q-a)*(s/Ye),i=x-(x-i)*(s/Ye),a+=m-ne,i+=E-oe,te=u,ne=m,oe=E,g(),L()}},Le=e=>{e.touches.length<2&&(N=!1)};t.addEventListener("touchstart",pe,{passive:!1}),t.addEventListener("touchmove",ge,{passive:!1}),t.addEventListener("touchend",Le);let Me=1,Te=e=>{e.preventDefault(),!N&&(P=!0,Me=f)},ye=e=>{if(e.preventDefault(),!P)return;let o=e,r=t.getBoundingClientRect();f=Math.max(H,Math.min(X,Me*o.scale)),R=o.clientX-r.left,W=o.clientY-r.top,d||(d=requestAnimationFrame($))},be=e=>{e.preventDefault(),P=!1};t.addEventListener("gesturestart",Te),t.addEventListener("gesturechange",ye),t.addEventListener("gestureend",be);let we=.8,Ze=2,Fe=e=>{let o=document.elementFromPoint(e.clientX,e.clientY);if(!(o instanceof HTMLElement)||o.closest("a")||o.closest("button"))return;let r=o.closest(".canvas-node");if(!r){j(S,I,A);return}let u=t.getBoundingClientRect(),m=r.getBoundingClientRect(),E=(m.left-u.left-a)/s,V=(m.top-u.top-i)/s,q=m.width/s,x=m.height/s,T=Math.min(u.width/q*we,u.height/x*we,Ze);j(T,(u.width-q*T)/2-E*T,(u.height-x*T)/2-V*T)};t.addEventListener("dblclick",Fe),h.push(()=>{t.removeEventListener("wheel",fe),t.removeEventListener("pointerdown",me),t.removeEventListener("pointermove",ve),t.removeEventListener("pointerup",G),t.removeEventListener("pointercancel",G),t.removeEventListener("dragstart",he),t.removeEventListener("touchstart",pe),t.removeEventListener("touchmove",ge),t.removeEventListener("touchend",Le),t.removeEventListener("gesturestart",Te),t.removeEventListener("gesturechange",ye),t.removeEventListener("gestureend",be),t.removeEventListener("dblclick",Fe)})}let J=p?.querySelector(".canvas-sidebar-toggle");if(p&&J){let n=()=>{p.classList.toggle("canvas-sidebar-open")};J.addEventListener("click",n),h.push(()=>{J.removeEventListener("click",n)})}let Q=t.querySelector(".canvas-zoom-in"),ee=t.querySelector(".canvas-zoom-out"),de=n=>{let c=t.getBoundingClientRect();U(n,c.width/2,c.height/2)};if(h.push(()=>{d&&cancelAnimationFrame(d),b()}),Q){let n=()=>{de(1.25)};Q.addEventListener("click",n),h.push(()=>Q.removeEventListener("click",n))}if(ee){let n=()=>{de(.8)};ee.addEventListener("click",n),h.push(()=>ee.removeEventListener("click",n))}if(C){let n=()=>{j(S,I,A)};C.addEventListener("click",n),h.push(()=>C.removeEventListener("click",n))}let O=t.querySelector(".canvas-fullscreen-toggle");if(O){let n=O.querySelector(".canvas-fullscreen-enter"),c=O.querySelector(".canvas-fullscreen-exit"),l=()=>{let v=document.fullscreenElement===t;n&&(n.style.display=v?"none":""),c&&(c.style.display=v?"":"none")},w=()=>{document.fullscreenElement===t?document.exitFullscreen():t.requestFullscreen()},F=()=>{l(),requestAnimationFrame(()=>{K(),S=s,I=a,A=i,L()})};O.addEventListener("click",w),document.addEventListener("fullscreenchange",F),h.push(()=>{O.removeEventListener("click",w),document.removeEventListener("fullscreenchange",F)})}let Ce=t.querySelectorAll(".canvas-iframe-wrapper iframe");for(let n of Array.from(Ce))n.addEventListener("error",()=>{let c=n.parentElement?.querySelector(".canvas-iframe-fallback");c&&(n.style.display="none",c.style.display="flex")});typeof window<"u"&&window.addCleanup&&window.addCleanup(()=>{for(let n of h)n();t.dataset.initialized="false"})}}if(typeof document<"u"){let Y=()=>{Be()};document.addEventListener("nav",Y),document.addEventListener("render",Y)}\n';
var l;
