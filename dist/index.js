...
.canvas-node-content > :first-child {
  margin-top: 0;
}
...
.canvas-node-content h1:first-child,
.canvas-node-content h2:first-child,
.canvas-node-content h3:first-child,
.canvas-node-content h4:first-child,
.canvas-node-content h5:first-child,
.canvas-node-content h6:first-child {
  margin-top: 0;
}
.canvas-node-content h1 {
  font-size: 1.5em;
...
.canvas-node-content::-webkit-scrollbar {
  width: 6px;
  height: 6px;
}
.canvas-node-content::-webkit-scrollbar-track {
  background: transparent;
}
.canvas-node-content::-webkit-scrollbar-thumb {
  background: transparent;
  border-radius: 3px;
}
.canvas-node:hover .canvas-node-content {
  scrollbar-color: var(--gray) transparent;
}
.canvas-node:hover .canvas-node-content::-webkit-scrollbar-thumb {
  background: var(--gray);
}
...
.canvas-node-content.canvas-content-scrollable.canvas-content-at-start {
  --fade-start: black;
}
.canvas-node-content.canvas-content-scrollable.canvas-content-at-end {
  --fade-end: black;
}
...
.canvas-node-file .canvas-embed-content > :first-child {
  margin-top: 0;
}
.canvas-node-file .canvas-embed-content > :last-child {
  margin-bottom: 0;
}
...
.canvas-node-group {
  background: color-mix(in srgb, var(--canvas-node-color, var(--gray)) 8%, transparent);
  border-style: solid;
  border-width: 1px;
...
var canvas_inline_default = 'function ke(){let Y=document.querySelectorAll(".canvas-container");if(Y.length!==0)for(let n of Array.from(Y)){if(n.dataset.initialized==="true")continue;n.dataset.initialized="true";let z=n.querySelector(".canvas-viewport");if(!z)continue;let Oe=n.dataset.enableInteraction!=="false",qe=n.dataset.defaultFullscreen==="true",f=n.closest(\'.page[data-frame="canvas"]\'),xe=window.matchMedia("(max-width: 800px)").matches;f&&!qe&&!xe&&(f.classList.add("canvas-no-transition","canvas-sidebar-open"),f.offsetWidth,requestAnimationFrame(()=>{f.classList.remove("canvas-no-transition")}));let C=parseFloat(n.dataset.minZoom??"")||.1,X=parseFloat(n.dataset.maxZoom??"")||5,Ze=parseFloat(n.dataset.initialZoom??"")||1,a=1,m=a,B=0,W=0,u=0,H=0,i=0,l=0,k=!1,se=0,ae=0,g=()=>{z.style.transform=`translate(${i}px, ${l}px) scale(${a})`},re=.9,ce=()=>{let t=n.getBoundingClientRect(),s=parseFloat(z.style.width)||1e3,c=parseFloat(z.style.height)||1e3,h=t.width/s*re,E=t.height/c*re,d=Math.min(h,E,1)*Ze;return d=Math.max(C,Math.min(X,d)),{zoom:d,panX:(t.width-s*d)/2,panY:(t.height-c*d)/2}},w=()=>{H&&(cancelAnimationFrame(H),H=0)},G=()=>{let t=ce();a=t.zoom,i=t.panX,l=t.panY,m=a,u&&(cancelAnimationFrame(u),u=0),w(),g()};G();let A=a,R=i,I=l,O=n.querySelector(".canvas-reset-view"),ie=()=>Math.abs(a-A)<=.001&&Math.abs(i-R)<=1&&Math.abs(l-I)<=1,L=()=>{O&&(O.style.display=ie()?"none":"flex")},_=.2,V=()=>{let t=a;a+=(m-a)*_,Math.abs(m-a)<5e-4&&(a=m),i=B-(B-i)*(a/t),l=W-(W-l)*(a/t),g(),L(),u=a===m?0:requestAnimationFrame(V)},$=(t,s,c)=>{w(),m=Math.max(C,Math.min(X,m*t)),B=s,W=c,u||(u=requestAnimationFrame(V))},q=1,x=0,Z=0,le=()=>{a+=(q-a)*_,i+=(x-i)*_,l+=(Z-l)*_,Math.abs(q-a)<5e-4&&Math.abs(x-i)<.5&&Math.abs(Z-l)<.5&&(a=q,i=x,l=Z),m=a,g(),L(),H=a===q&&i===x&&l===Z?0:requestAnimationFrame(le)},U=(t,s,c)=>{q=Math.max(C,Math.min(X,t)),x=s,Z=c,u&&(cancelAnimationFrame(u),u=0),H||(H=requestAnimationFrame(le))},ue=.8,De=2,de=t=>{let s=n.getBoundingClientRect(),c=t.getBoundingClientRect(),h=(c.left-s.left-i)/a,E=(c.top-s.top-l)/a,d=c.width/a,T=c.height/a,p=Math.min(s.width/d*ue,s.height/T*ue,De);U(p,(s.width-d*p)/2-h*p,(s.height-T*p)/2-E*p)},K=t=>{let s=location.pathname+location.search;history.replaceState(null,"",t?`${s}#node=${encodeURIComponent(t)}`:s)},v=[],M=n.getBoundingClientRect(),ze=()=>{let t=n.getBoundingClientRect();if(t.left===M.left&&t.top===M.top&&t.width===M.width&&t.height===M.height)return;let s=ie();i-=t.left-M.left,l-=t.top-M.top,M=t;let c=ce();A=c.zoom,R=c.panX,I=c.panY,s?G():g(),L()},fe=new ResizeObserver(ze);if(fe.observe(n),v.push(()=>fe.disconnect()),f){let t=window.matchMedia("(max-width: 800px)"),s=c=>{c.matches&&(f.classList.add("canvas-no-transition"),f.classList.remove("canvas-sidebar-open"),f.offsetWidth,requestAnimationFrame(()=>f.classList.remove("canvas-no-transition")))};t.addEventListener("change",s),v.push(()=>t.removeEventListener("change",s))}let F=4,me=t=>{t.classList.toggle("canvas-content-at-start",t.scrollTop<=2),t.classList.toggle("canvas-content-at-end",t.scrollTop+t.clientHeight>=t.scrollHeight-2)},ve=t=>{t.classList.toggle("canvas-content-scrollable",t.scrollHeight-t.clientHeight>F),me(t)};for(let t of Array.from(n.querySelectorAll(".canvas-node-content")))t instanceof HTMLElement&&ve(t);let he=t=>{t.target instanceof HTMLElement&&t.target.classList.contains("canvas-node-content")&&me(t.target)},Ee=t=>{let s=t.target instanceof HTMLElement?t.target.closest(".canvas-node-content"):null;s instanceof HTMLElement&&ve(s)};if(n.addEventListener("scroll",he,!0),n.addEventListener("load",Ee,!0),v.push(()=>{n.removeEventListener("scroll",he,!0),n.removeEventListener("load",Ee,!0)}),Oe){let c=e=>e.deltaMode!==0?e.deltaY>0?.7692307692307692:1.3:Math.exp(-e.deltaY*.004),h=e=>e.deltaMode!==0?!1:e.deltaX!==0||!Number.isInteger(e.deltaY)?!0:Math.abs(e.deltaY)<40,E=(e,o)=>Math.abs(o.deltaX)>Math.abs(o.deltaY)?e.scrollWidth>e.clientWidth+F:e.scrollHeight>e.clientHeight+F,d=(e,o)=>Math.abs(o.deltaX)>Math.abs(o.deltaY)?!(e.scrollWidth<=e.clientWidth+F||e.scrollLeft<=0&&o.deltaX<0||e.scrollLeft+e.clientWidth>=e.scrollWidth-1&&o.deltaX>0):!(e.scrollHeight<=e.clientHeight+F||e.scrollTop<=0&&o.deltaY<0||e.scrollTop+e.clientHeight>=e.scrollHeight-1&&o.deltaY>0),T=!1,p=e=>{if(T){e.preventDefault();return}let o=n.getBoundingClientRect(),r=e.clientX-o.left,y=e.clientY-o.top;if(e.ctrlKey||e.metaKey){e.preventDefault(),$(c(e),r,y);return}let S=e.target instanceof HTMLElement?e.target.closest(".canvas-node:not(.canvas-node-group)"):null;if(S){let b=S.querySelector(".canvas-node-content");if(b instanceof HTMLElement&&E(b,e)){if(d(b,e))return;e.preventDefault();return}}e.preventDefault(),h(e)?(w(),i-=e.deltaX,l-=e.deltaY,g(),L()):$(c(e),r,y)},ge=e=>{let o=e.button===1;if(!(e.button!==0&&!o)){if(o)e.preventDefault();else if(e.target instanceof HTMLElement){if(e.target.closest("a")||e.target.closest("button"))return;let r=e.target.closest(".canvas-node-content");if(r&&r.scrollHeight>r.clientHeight+F){let y=r.getBoundingClientRect();if(e.clientX>=y.right-16)return}}w(),k=!0,se=e.clientX-i,ae=e.clientY-l,n.style.cursor="grabbing",n.setPointerCapture(e.pointerId)}},Le=e=>{k&&(i=e.clientX-se,l=e.clientY-ae,g(),L())},P=()=>{k=!1,n.style.cursor=""},Me=e=>e.preventDefault();n.addEventListener("wheel",p,{passive:!1}),n.addEventListener("pointerdown",ge),n.addEventListener("pointermove",Le),n.addEventListener("pointerup",P),n.addEventListener("pointercancel",P),n.addEventListener("dragstart",Me);let te=0,ne=0,oe=0,N=!1,Te=e=>{if(e.length<2||!e[0]||!e[1])return 0;let o=e[0].clientX-e[1].clientX,r=e[0].clientY-e[1].clientY;return Math.sqrt(o*o+r*r)},ye=e=>{if(e.touches.length===2){let o=e.touches[0],r=e.touches[1];if(!o||!r)return;e.preventDefault(),w(),N=!0,k=!1,te=Te(e.touches),ne=(o.clientX+r.clientX)/2,oe=(o.clientY+r.clientY)/2}},be=e=>{if(e.touches.length===2&&N){let o=e.touches[0],r=e.touches[1];if(!o||!r)return;e.preventDefault();let y=Te(e.touches),S=(o.clientX+r.clientX)/2,b=(o.clientY+r.clientY)/2,Xe=n.getBoundingClientRect(),Ae=S-Xe.left,Re=b-Xe.top,We=y/te,Ie=a;a=Math.max(C,Math.min(X,a*We)),m=a,i=Ae-(Ae-i)*(a/Ie),l=Re-(Re-l)*(a/Ie),i+=S-ne,l+=b-oe,te=y,ne=S,oe=b,g(),L()}},He=e=>{e.touches.length<2&&(N=!1)};n.addEventListener("touchstart",ye,{passive:!1}),n.addEventListener("touchmove",be,{passive:!1}),n.addEventListener("touchend",He);let we=1,Fe=e=>{e.preventDefault(),!N&&(T=!0,we=m)},Se=e=>{if(e.preventDefault(),!T)return;let o=e,r=n.getBoundingClientRect();m=Math.max(C,Math.min(X,we*o.scale)),B=o.clientX-r.left,W=o.clientY-r.top,u||(u=requestAnimationFrame(V))},Ye=e=>{e.preventDefault(),T=!1};n.addEventListener("gesturestart",Fe),n.addEventListener("gesturechange",Se),n.addEventListener("gestureend",Ye);let Ce=e=>{let o=document.elementFromPoint(e.clientX,e.clientY);if(!(o instanceof HTMLElement)||o.closest("a")||o.closest("button"))return;let r=o.closest(".canvas-node");if(!r){U(A,R,I),K();return}de(r),K(r.dataset.nodeId)};n.addEventListener("dblclick",Ce),v.push(()=>{n.removeEventListener("wheel",p),n.removeEventListener("pointerdown",ge),n.removeEventListener("pointermove",Le),n.removeEventListener("pointerup",P),n.removeEventListener("pointercancel",P),n.removeEventListener("dragstart",Me),n.removeEventListener("touchstart",ye),n.removeEventListener("touchmove",be),n.removeEventListener("touchend",He),n.removeEventListener("gesturestart",Fe),n.removeEventListener("gesturechange",Se),n.removeEventListener("gestureend",Ye),n.removeEventListener("dblclick",Ce)})}let Q=f?.querySelector(".canvas-sidebar-toggle");if(f&&Q){let t=()=>{f.classList.toggle("canvas-sidebar-open")};Q.addEventListener("click",t),v.push(()=>{Q.removeEventListener("click",t)})}let j=n.querySelector(".canvas-zoom-in"),J=n.querySelector(".canvas-zoom-out"),pe=t=>{let s=n.getBoundingClientRect();$(t,s.width/2,s.height/2)};if(v.push(()=>{u&&cancelAnimationFrame(u),w()}),j){let t=()=>{pe(1.25)};j.addEventListener("click",t),v.push(()=>j.removeEventListener("click",t))}if(J){let t=()=>{pe(.8)};J.addEventListener("click",t),v.push(()=>J.removeEventListener("click",t))}if(O){let t=()=>{U(A,R,I),K()};O.addEventListener("click",t),v.push(()=>O.removeEventListener("click",t))}let D=n.querySelector(".canvas-fullscreen-toggle");if(D){let t=D.querySelector(".canvas-fullscreen-enter"),s=D.querySelector(".canvas-fullscreen-exit"),c=()=>{let d=document.fullscreenElement===n;t&&(t.style.display=d?"none":""),s&&(s.style.display=d?"":"none")},h=()=>{document.fullscreenElement===n?document.exitFullscreen():n.requestFullscreen()},E=()=>{c(),requestAnimationFrame(()=>{G(),A=a,R=i,I=l,L()})};D.addEventListener("click",h),document.addEventListener("fullscreenchange",E),v.push(()=>{D.removeEventListener("click",h),document.removeEventListener("fullscreenchange",E)})}let ee=/^#node=([^&]+)/.exec(location.hash);if(ee&&ee[1]){let t=n.querySelector(`.canvas-node[data-node-id="${CSS.escape(decodeURIComponent(ee[1]))}"]`);t instanceof HTMLElement&&de(t)}let Be=n.querySelectorAll(".canvas-iframe-wrapper iframe");for(let t of Array.from(Be))t.addEventListener("error",()=>{let s=t.parentElement?.querySelector(".canvas-iframe-fallback");s&&(t.style.display="none",s.style.display="flex")});typeof window<"u"&&window.addCleanup&&window.addCleanup(()=>{for(let t of v)t();n.dataset.initialized="false"})}}if(typeof document<"u"){let Y=()=>{ke()};document.addEventListener("nav",Y),document.addEventListener("render",Y)}\n';
var l;
...
function renderEdge(edge, nodeMap) {
  ...
  return /* @__PURE__ */ u2("g", { class: "canvas-edge", "data-edge-id": edge.id, children: [
    ...
    edge.label && /* @__PURE__ */ u2("text", { x: midX, y: midY, class: "canvas-edge-label", "text-anchor": "middle", dy: "-8", children: edge.label })
  ] });
}
...
function findVaultPage(target, canvasPath, allFiles) {
  const stripExt = (f3) => f3.replace(/\.md$/i, "");
  const norm = stripExt(target.replace(/^\.?\//, ""));
  const lower = norm.toLowerCase();
  const exact = allFiles.find((f3) => stripExt(f3).toLowerCase() === lower);
  if (exact) return exact;
  if (norm.includes("/")) {
    const suffix = allFiles.find((f3) => stripExt(f3).toLowerCase().endsWith(`/${lower}`));
    if (suffix) return suffix;
  }
  const dir = canvasPath.split("/").slice(0, -1).join("/");
  if (dir) {
    const sameFolder = `${dir}/${norm}`.toLowerCase();
    const hit = allFiles.find((f3) => stripExt(f3).toLowerCase() === sameFolder);
    if (hit) return hit;
  }
  const wantBase = lower.split("/").pop();
  const matches = allFiles.filter(
    (f3) => stripExt(f3.split("/").pop() ?? "").toLowerCase() === wantBase
  );
  return matches.find((f3) => /\.md$/i.test(f3)) ?? matches[0];
}
var escapeAttr = (s2) => s2.replace(/&/g, "&amp;").replace(/"/g, "&quot;");
var escapeText = (s2) => s2.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;");
function resolveWikiLinks(text5, canvasPath, canvasSlug, allFiles) {
  return text5.replace(
    ...
    (whole, rawTarget, rawSub, rawAlias) => {
      ...
      const slug2 = slugifyFilePath(page);
      let href = resolveRelative(canvasSlug, slug2);
      const sub = rawSub?.trim();
      if (sub) href += sub.startsWith("^") ? `#${sub}` : `#${new BananaSlug().slug(sub)}`;
      const label = (rawAlias?.trim() || (sub ? `${target} \u203A ${sub}` : target)).trim();
      return `<a href="${escapeAttr(href)}" class="internal" data-slug="${escapeAttr(slug2)}">${escapeText(label)}</a>`;
    }
  );
}
function renderMarkdown(text5, canvasPath, canvasSlug, allFiles) {
  const withEmbeds = resolveWikiImageEmbeds(text5, canvasPath, canvasSlug, allFiles);
  const withLinks = resolveWikiLinks(withEmbeds, canvasPath, canvasSlug, allFiles);
  const html5 = micromark(withLinks, {
