...
- **Obsidian-like pan & zoom.** A trackpad two-finger swipe pans and a pinch zooms (including in Safari, which reports pinches via gesture events rather than wheel events); a mouse wheel zooms at the cursor. A wheel or swipe over a card scrolls that card's content and no longer "escapes" to pan or zoom the whole canvas once the card reaches its scroll edge. On touch screens, a drag that starts on a card pans the canvas instead of being hijacked by the browser's native scrolling, while a genuinely scrollable card still scrolls (and pins at its edge, without triggering pull-to-refresh).
